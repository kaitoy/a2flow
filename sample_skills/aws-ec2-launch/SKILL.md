...
`workflow_task_id` names the task the approval takes effect **from**, so name **this approval step**. The approval then covers it and every task after it — the launch included — up to the next approval, and none of them may call an MCP tool until the decision is `approved`. Call `list_workflow_tasks` to find this step's id.

```
list_workflow_tasks()
list_user_groups()
request_approval(title="Launch EC2 instance: <name>", workflow_task_id=<approval_task_id>, approver_group_id=<group_id>, description="<finalized configuration>")
