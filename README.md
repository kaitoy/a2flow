...
Because that one chat is shared, several people (the **applicant**/owner, designated **approvers**, and the **agent**) post into it, so each message carries a **sender avatar** to show who sent it — the applicant's or approver's avatar beside their messages, and an agent badge (hover shows the workflow name) beside the agent's. ADK records every human message under the author `user`, so the backend attributes each one to its real sender: the agent run endpoint snapshots the session's existing `user` events and records the current user as the sender of any new ones, and `GET /workflow-sessions/{id}/messages` returns each message's `senderUserId`. Messages with no recorded sender (history sent before attribution existed) fall back to the owner. Hovering an avatar reveals the sender's name.
...
The chat screen also shows a collapsible **task timeline** down the left edge: the session's `WorkflowTask`s in order, each with a status dot, with the in-progress task highlighted. The agent drives the task lifecycle by calling `update_workflow_task(status="in_progress")` before working on a task, so after each run the backend walks the session's events in order, tracks the most recent such transition, and records the in-progress task for every following message as that message's `workflow_task_id`. `GET /workflow-sessions/{id}/messages` returns each message's `workflowTaskId`, and the chat inserts a **task divider** wherever it changes — grouping the messages under the task that produced them. Clicking a task in the timeline scrolls the chat to that task's first message.
