...
| **[Append-only audit logs](https://kaitoy.github.io/a2flow/docs/guides/audit-logs)** | Every tool call the gateway allowed or refused, every certificate issued, every impersonated session, every mail sent. Nothing in the screens edits or removes a record. |
...
the chat, and what the reader does with them comes back as the next turn. The agent works
from an [Agent Skill](https://kaitoy.github.io/a2flow/docs/guides/agent-skills) — a
`SKILL.md` in an external Git repository that A2Flow clones and keeps current — turning it
into a task graph it can run. Every record lives in PostgreSQL, the audit log among them,
append-only. A secret is held by name and
