...
Per-message side-channel facts like this live in a single **`message_meta`** table keyed by `(workflow_execution_id, adk_event_id)`, with nullable columns the run-completion step upserts independently — currently `sender_user_id` (above) and `workflow_task_id` (below).
