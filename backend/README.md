...
- **List parameters** — collection endpoints accept shared `limit` / `offset` / sort (`s`) / filter (`q`) query parameters with camelCase field names. The six taggable collections additionally accept a repeatable `tag` parameter carrying tag ids; a record must carry **every** id listed to match (see [Tags](#tags)).
...
A tag is a tenant-scoped label — a `name` unique within the tenant plus a `color` naming one of eight fixed palette slots — that secrets, workflows, MCP servers, agent skills, tool mocks, and user groups are classified by. One vocabulary serves all six. CRUD endpoints are in the [API reference](http://localhost:3000/api-doc); writes require `admin` **or** `developer`, since secrets are administered by the former and the other resources by the latter — except that creating, editing, or deleting a tag that is (or would become) an access-control gate is `admin`-only (`services/tag.py::TagService`; a `developer` gets `403 FORBIDDEN` on the whole write, not just on a change to the flag). A plain tag's create/edit/delete stays open to `developer`.

Attachment lives in one join table per resource type (`secret_tags`, `workflow_tags`, `mcp_server_tags`, `agent_skill_tags`, `mcp_tool_mock_tags`, `user_group_tags`), each keyed by the record's id and the tag's **id** — never its name, which is what makes a rename free of any re-sync. Six tables rather than one polymorphic table because a polymorphic owner column cannot carry a real foreign key; with real ones, `ondelete="CASCADE"` on both sides cleans up an attachment when either the record or the tag is deleted. Deleting a tag therefore detaches it everywhere instead of being refused.

A tag with `accessControl: true` also **gates** the records it labels. A caller may list, read, edit, delete, execute, or otherwise act on such a record only when the tags attached to the caller's user groups, taken together, include every access-control tag the record carries; otherwise the record is missing from list responses and reads as `404 NOT_FOUND` by id, so its existence is never confirmed. A `super_admin` is exempt (platform-scoped, so never a group member). Unflagged tags remain plain filters. A workflow execution snapshots its workflow's tags when it starts and its approvals carry no tags of their own, so the same rule reaches a run through the copied tags: driving its agent, reading its tasks, or deciding an approval under it all read as `404 NOT_FOUND` for a caller whose groups have since lost such a tag, even the run's initiator or the approval's designated approver. Two things stay outside the rule on purpose: resolving a `${secret:NAME/KEY}` placeholder or a run's agent skill *on behalf of* a workflow or MCP server (a requester need not hold the secret's tag for the workflow to run), and foreign-key existence checks. `PUT …/tags` refuses, with `403 FORBIDDEN`, an access-control tag the caller's groups do not carry — attaching it would hide the record from the caller on their next request — except on a user group, since tagging a group is how an admin grants access. The enforcement lives in the repository layer next to tenant scoping; see "Access-control tags in the Repository Layer" in [.claude/rules/backend-patterns.md](../.claude/rules/backend-patterns.md).
...
A `${gcp-token:NAME/KEY}` placeholder works in the same places and fails the same way, but instead of the entry's value it expands to a Google OAuth 2.0 **access token** minted from it (`infrastructure/google_token.py`). The entry must hold a Google credential JSON: a service account key (tokens are minted with the `cloud-platform` scope), or the `authorized_user` JSON `gcloud auth application-default login --client-id-file=… --scopes=…` writes, whose refresh token carries the scopes consent granted. This is how a Google Cloud remote MCP server such as GKE's (`https://container.googleapis.com/mcp`) is authenticated — they reject API keys and do not support Dynamic Client Registration, so the MCP SDK's own `OAuthClientProvider` is not used. The parsed credential is cached per process on the JSON text and google-auth refreshes its token when it expires, so the token endpoint sees roughly one call an hour per credential, not one per tool call; the expansion runs before the `${secret:…}` pass, and the minted token crosses to the [MCP proxy](#mcp-proxy) as an already-resolved header value.
...
`POST /api/v1/workflows/{id}/publish` (reviewer-gated) makes a workflow executable: it requires at least one task template and no generation in flight (`409 WORKFLOW_NOT_RUNNABLE` otherwise) and **freezes the design** into the workflow's `WorkflowPublishedVersion` row (name, description, and every task template with its edges and tool bindings — replacing the previous snapshot), and sets `status: "published"`. Executing a workflow — `POST /api/v1/workflows/{id}/execute`, requester-gated — accepts published, `modified`, and (developer-only) draft workflows (`409 WORKFLOW_NOT_RUNNABLE` otherwise), snapshots its configuration into a new `WorkflowExecution`, and copies its templates into the execution's tasks (see below). The remaining endpoints (list/get/patch/delete, `GET /{id}/task-templates`) are in the [API reference](http://localhost:3000/api-doc); `GET /{id}/messages` and `POST /{id}/agent` serve the workflow's [design session](#design-sessions).
...
A workflow task is a single actionable item belonging to a `WorkflowExecution`, copied from the workflow's task templates at execute time and driven by the execution agent via [agent tools](#agent-task-tools). A run's task list is fixed once it is created: there is **no REST create or delete endpoint**, and `PATCH /workflow-tasks/{id}` touches only `status` / `errorKind` / `errorMessage` — a task's `title`, `description`, `dependsOnIds` and `toolBindings` are copied from the template and never change afterwards. Each task carries a `status` (`pending` | `in_progress` | `completed` | `failed` | `skipped`); tasks are listed in `createdAt` order. Deleting the parent `WorkflowExecution` cascades to its tasks.

Tasks form a **directed acyclic graph (DAG)**: each task may depend on other tasks in the same session through its `dependsOnIds` list (persisted as `(task_id, depends_on_id)` rows in the `workflow_task_dependencies` join table, where `depends_on_id` must precede `task_id`). Read responses include the resolved `dependsOnIds`. The edges are built when the templates are copied in (dependency targets must exist and belong to the same session, and a cycle — including a self-dependency — is rejected before the run is created) and cascade-delete with the parent execution.

Tasks may additionally bind **MCP tools** from [registered MCP servers](#mcp-servers) through their `toolBindings` list (`[{"mcpServerId": …, "toolName": …}]`, persisted in the `workflow_task_tool_bindings` join table), also copied from the template at execute time. Read responses include the resolved `toolBindings`. Bindings cascade-delete with their task, while a referenced MCP server cannot be deleted (`409 CONFLICT_REFERENCED`). At execution time the agent may only invoke bound tools via the `call_mcp_tool` proxy (see [Agent task tools](#agent-task-tools)).
...
The task read and status-update endpoints — list-for-an-execution (ordered `created_at` ASC then `id` ASC), get, and `PATCH` — are in the [API reference](http://localhost:3000/api-doc). A rule the spec does not spell out: `WorkflowTaskUpdate` carries only `status` / `errorKind` / `errorMessage`, so a `PATCH` body that also sets `title`, `description`, `dependsOnIds`, `toolBindings` or `workflowExecutionId` is accepted but those fields are silently ignored — a run's task list is not editable through the API.
...
- **The third policy** is `ApprovedArgumentsPolicy`, the argument gate. Where `TaskCertificatePolicy` establishes that something signed authorized this *tool* for this task, this one holds the *call* to what the approver approved: each governing approval's `approvedCalls` declaration is matched against `ctx.arguments` by `infrastructure/approved_calls.py`. **Every** governing approval must permit the call, matching the rule `_assert_grantor_still_authorizes` already applies to their statuses — a merge of two gated branches is authorized by both approvers or by neither. An initiator grant (`binding.approval_id is None`) is skipped: there is no approver to have deviated from, and `TaskCertificatePolicy` has already refused an initiator grant for any approval-governed task. An approval with an empty declaration is skipped too, which is what makes the rule safe to deploy over a running system; nothing new can reach that state, since `request_approval` now requires a declaration covering every tool the covered tasks bind that requires input approval. A tool the design exempted needs nothing of this policy either: `request_approval` recorded it in the declaration as an `unconstrainedArguments` entry, and `match_call` admits it there — so this rule never reads a binding's flag, and the declaration on the row stays the only thing it consults. Registered **last**: it is the most expensive rule, and it is the only one needing a `task_id` it can trust — the binding policy allows the *union* of every in-progress task's bindings, so only after the certificate check is `binding.task_id` something to resolve an approval from. Running it last also keeps a caller with no authority at all from learning what a declaration says.
...
| `infrastructure/mcp_connection.py` | Resolves an `MCPServer` row and its `${secret:…}` / `${gcp-token:…}` placeholders into a connection spec. Stays on this side: only the resolved values for the one server being called cross over, which is why the far side needs neither the Fernet key, nor Vault, nor a Google credential |
...
- **For MCP tools the stub sits inside the gateway, behind the policy chain.** `WorkflowExecutionToolStub` (`infrastructure/tool_mocks.py`) implements the gateway's `McpToolStub` hook, which `call_tool` consults only *after* the [policy chain](#mcp-gateway) has allowed the call. A stubbed run therefore rehearses the real one: the tool must still be bound to a task the run has in progress, and the call must still present that task's certificate. What the mock skips is the one thing with an effect outside A2Flow — the upstream call. The built-in `request_approval` calls `resolve_mock` directly instead, since it writes to `approvals` rather than reaching a server.
- **A stubbed call leaves no `mcp_tool_invocations` row, allowed or refused.** That table records the calls that reached (or were stopped on their way to) a real MCP server; a row for a call that was always going to be answered from a snapshot would misread in either direction. This is why the stub hook has two methods — the gateway must know whether a call is stubbed *before* deciding whether to audit a refusal, and finding that out must not consume one of the run's ordered responses.
...
- **A covered task's `tool_bindings` are frozen** — as are every task's, in fact: they are copied from the workflow's template at execute time and the REST API exposes no way to change them (or a task's title, description or dependencies) on a run. This closes the gap a lazily-signed certificate would otherwise open, where an edit between the decision and the covered task's start could widen what the decision goes on to authorize.
...
**One root for the whole platform.** A per-tenant CA would add key material without adding a boundary: verification compares the tenant in the certificate's binding URN against the tenant the gateway derived independently from the session id, so a certificate minted for tenant A is refused in tenant B regardless of which key signed it. The root's private key is stored as Fernet ciphertext under the same key as [local secrets](#secrets); losing that key stops new certificates from being issued.
...
`mcp_tool_invocations` and `mcp_tool_certificates` rows carry `workflowExecutionId`, `workflowTaskId`, and `approvalId` but no inlined name for any of them. The frontend resolves `workflowExecutionId`/`approvalId` through the already-flat, already-tenant-wide `GET /workflow-executions` and `GET /approvals`, filtered by `id:in:`. `WorkflowTask` had no equivalent flat surface — only `GET /workflow-tasks/{id}` and the execution-scoped `GET /workflow-executions/{id}/workflow-tasks` — so `GET /workflow-tasks` (`routers/workflow_tasks.py`) was added, gated the same `admin`-or-`super_admin` way as the three surfaces above, for the same `id:in:` name-resolution use.
...
`approvedCalls` is what the decision is *about*, and the argument-level counterpart to the certificate's frozen tool set: a list of `{mcpServerId, toolName, arguments, unconstrainedArguments}` entries, where each argument name maps to a constraint carrying exactly one of `eq` / `in` / `lte` / `gte` / `matches` plus an optional `optional` modifier. `unconstrainedArguments` marks a tool the workflow's design exempted from input approval (`ToolBinding.requiresInputApproval`, set on a task template and copied onto the run's tasks): it permits any arguments, carries no `arguments` of its own, and is written **only** by `request_approval` — an agent-supplied entry setting it is refused, so a run cannot exempt itself. It is declared on `ApprovalCreate` but **redeclared on the table class** as `list[dict[str, Any]]` over a `JSONColumn` — SQLAlchemy's JSON serializer cannot encode a Pydantic model, the same reason `MCPToolMock.responses` is declared that way — and `ApprovalRead` restores the typed shape for `GET /api/v1/approvals/{id}` so the generated frontend bindings describe it. The list endpoint deliberately keeps the table class: a declaration is detail, and nothing filters or sorts on it.

Because the field is absent from `ApprovalUpdate` and there is no `POST /approvals`, it is unwritable through the API by construction — the same technique `decidedAt` / `decidedBy` use — so `request_approval` is its only writer. Shape validation therefore lives in `infrastructure/approved_calls.py::validate_declaration`, called from `request_approval`, and **not** in a model validator: `Approval` inherits `ApprovalCreate`'s validators and FastAPI runs them on the way *out* too, so a rule rejecting a legacy shape would turn an old row from readable into an HTTP 500. `request_approval` also requires the declaration to name **exactly** the tools bound to the tasks the prospective approval would cover **that require input approval** — computed by `_prospective_tool_bindings`, which runs `covered_task_ids` over an `active_approval_by_task` mapping with the named task pointed at a placeholder, so the not-yet-written request takes the task over exactly as the real row will. It returns the covered pairs split in two: a pair counts as exempt only when *every* covered binding of it clears `requiresInputApproval`, so the stricter step decides where two disagree. The exempt pairs must be left out of the request, and `request_approval` appends an `unconstrainedArguments` entry for each before writing the row — which is what keeps the stored declaration the complete list of what the decision authorizes, for the approver reading it and for the gate matching against it. An empty declaration is accepted only when no covered task binds anything requiring input approval. Both checks run **before** the tool-mock short-circuit: a mock skips the side effects, not the validation.
