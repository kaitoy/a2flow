...
- **Validation still runs.** A mocked `request_approval` still checks that the destination is a real, eligible approver — a mock skips the side effects, not the checks — so a misconfigured workflow fails in a dry run the same way it would for real.
