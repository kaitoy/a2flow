...
class Settings(BaseSettings):
    ...
    Attributes:
        ...
        demo_data: Whether the demo dataset (sample Agent Skill, AWS MCP
            server, Cloud Logging MCP server, AWS and Google Cloud secrets, and
            approver/requester users) is registered in the seeded ``Default``
            tenant on startup. When false, any previously registered demo record
            is removed again — see ``infrastructure/demo_data.py``.
        ...
        demo_gcp_api_key: Google Cloud API key stored in the demo
            ``demo-gcp-credentials`` secret and sent by the demo Cloud Logging
            MCP server as its ``x-goog-api-key`` header; a placeholder is
            stored when unset.
