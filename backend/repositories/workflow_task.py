...
class SqlWorkflowTaskRepository:
    ...
    async def update(
        ...
        ``WorkflowTaskUpdate`` carries only ``status`` / ``error_kind`` /
        ``error_message`` — a run's task list is fixed at execute time, so the
        title, description, dependency edges and tool bindings a task was
        created with are never touched here. ``workflow_execution_id`` is not
        part of the payload either, so no parent re-validation is needed.
        """
