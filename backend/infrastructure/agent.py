...
EXECUTION_AGENT_INSTRUCTION = (
    ...
    "required and must be the id of the task that will PERFORM the approved "
    "action — the one with the relevant `tool_bindings` — which is often a later "
    "task than the one you are working on now: a step whose whole job is to ask "
    "for a go-ahead binds no tools, and naming it would authorize nothing. Look "
    "the id up with `list_workflow_tasks`. You must address the "
