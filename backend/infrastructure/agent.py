...
_DESIGN_REGISTRATION_RULES = (
    ...
    "register the task templates without any `tools` entries.\n\n"
    "For each tool you bind, decide whether a human should have to agree to the "
    'values it is called with. Add `"requires_input_approval": false` to a '
    "tool that only READS -- a listing, a lookup, a status check -- because at "
    "run time an approval has to spell out the arguments of every other bound "
    "tool before anyone decides on it, and for a read-only tool that is both "
    "pointless (nothing to weigh) and often impossible (the run may still be "
    "exploring). Such a tool is still covered by the approval and still waits "
...
EXECUTION_AGENT_INSTRUCTION = (
    ...
    "is provided as context). Begin executing immediately, without redesigning "
    "the tasks or asking anyone to approve them.\n\n"
    ...
    "all `completed`. If several are runnable, pick the one that appears first "
    "in the `list_workflow_tasks` result.\n"
    ...
    "pending approval and notify whoever can decide it. `workflow_task_id` is "
    "required and names the task the approval takes effect FROM: the approval "
    "covers that task and every task after it in the dependency graph, up to "
    "the next approval. So when the task list has a step whose whole job is to "
    "ask for the go-ahead, name that step — the decision reaches the steps that "
    "follow it, and none of them may call an MCP tool until it is `approved`. "
    "Look the id up with `list_workflow_tasks`. You must also pass "
    "`approved_calls`, declaring every MCP call the approval authorizes: one "
    "entry per (server, tool) the covered tasks bind, and under `arguments` "
    "every argument you will send, each mapped to an object holding exactly one "
    'of `eq`, `in`, `lte`, `gte` or `matches` (add `"optional": true` for one '
    "you may omit). The approver reads this and decides on it, and the server "
    "then refuses any call that names an argument you did not declare, omits "
    "one you did, or passes a value outside the declared bounds -- so declare "
    "exactly the calls you intend to make, using `list_mcp_tools` to read each "
    "tool's input schema first. A bound tool whose `requires_input_approval` "
    "is false -- `list_workflow_tasks` reports the flag on every binding -- is "
    "the exception: leave it out of `approved_calls` entirely, and declaring it "
    "is refused. It is recorded as authorized with any arguments, and it still "
    "cannot be called until the approval is `approved`. You must address the "
    ...
    "approve/reject controls in the UI, rather than A2UI buttons. The "
