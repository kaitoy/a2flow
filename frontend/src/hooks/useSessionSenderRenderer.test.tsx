...
import {
  type SessionSenderRendererOptions,
  useSessionSenderRenderer,
} from "./useSessionSenderRenderer";
...
function renderer(overrides: Partial<SessionSenderRendererOptions> = {}) {
...
/** An A2UI surface activity produced by the given tool call. */
function a2uiMessage(toolCallId: string): Message {
  return {
    id: "m1",
    role: "activity",
    activityType: A2UIActivityType,
    content: { [A2UI_SOURCE_TOOL_CALL_ID_KEY]: toolCallId },
  } as unknown as Message;
}

/** An approval activity whose id is the `render_approval` tool call id. */
function approvalMessage(id: string): Message {
  return {
    id,
    role: "activity",
...
describe("useSessionSenderRenderer", () => {
  describe("renderAvatar", () => {
    it("labels the agent's own messages with the agent label", () => {
      const { renderAvatar } = renderer();
      render(
        <Rendered
          node={renderAvatar({ id: "m1", role: "assistant", content: "hello" } as Message)}
        />
      );
    ...
    it("resolves an A2UI surface by the tool call that produced it", () => {
      ...
      render(<Rendered node={renderAvatar(a2uiMessage("tc-1"))} />);
      expect(screen.getByTestId("avatar")).toHaveAttribute("data-user", OTHER.id);
    });

    it("shows no avatar on a surface nobody has acted on", () => {
      const { renderAvatar } = renderer();
      const { container } = render(<Rendered node={renderAvatar(a2uiMessage("tc-1"))} />);
    ...
    it("resolves an approval control to whoever decided it", () => {
      ...
      render(<Rendered node={renderAvatar(approvalMessage("tc-approve"))} />);
      expect(screen.getByTestId("avatar")).toHaveAttribute("data-user", OTHER.id);
    });
  });

  describe("isOwnMessage", () => {
    it("claims a message attributed to the viewer", () => {
      const { isOwnMessage } = renderer({ messageSenders: new Map([["m1", VIEWER.id]]) });
      expect(isOwnMessage(userMessage("m1"))).toBe(true);
    });

    it("claims a message this viewer just sent, before the history reloads", () => {
      const { isOwnMessage } = renderer({ locallySentMessageIds: new Set(["m1"]) });
      expect(isOwnMessage(userMessage("m1"))).toBe(true);
    });

    it("disclaims another participant's message", () => {
      const { isOwnMessage } = renderer({ messageSenders: new Map([["m1", OTHER.id]]) });
      expect(isOwnMessage(userMessage("m1"))).toBe(false);
    });

    it("disclaims unattributed history unless the viewer owns the session", () => {
      expect(renderer().isOwnMessage(userMessage("m1"))).toBe(false);
      expect(renderer({ currentUser: OWNER }).isOwnMessage(userMessage("m1"))).toBe(true);
    });

    it("never claims an agent message", () => {
      const { isOwnMessage } = renderer();
      expect(isOwnMessage({ id: "m1", role: "assistant", content: "hi" } as Message)).toBe(false);
    });

    it("claims an A2UI surface this viewer answered but not one someone else did", () => {
      expect(
        renderer({ messageSenders: new Map([["tc-1", VIEWER.id]]) }).isOwnMessage(
          a2uiMessage("tc-1")
        )
      ).toBe(true);
      expect(
        renderer({ messageSenders: new Map([["tc-1", OTHER.id]]) }).isOwnMessage(
          a2uiMessage("tc-1")
        )
      ).toBe(false);
    });

    it("claims an approval this viewer decided but not one someone else did", () => {
      expect(
        renderer({ messageSenders: new Map([["tc-a", VIEWER.id]]) }).isOwnMessage(
          approvalMessage("tc-a")
        )
      ).toBe(true);
      expect(
        renderer({ messageSenders: new Map([["tc-a", OTHER.id]]) }).isOwnMessage(
          approvalMessage("tc-a")
        )
      ).toBe(false);
    });

    it("claims nothing while the signed-in user is still unknown", () => {
      const { isOwnMessage } = renderer({
        currentUser: null,
        messageSenders: new Map([["m1", OWNER.id]]),
      });
      expect(isOwnMessage(userMessage("m1"))).toBe(false);
    });
  });
