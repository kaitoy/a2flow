...
- **[Tool mocks](../guides/tool-mocks.md)** — stubs for the demo run's side-effecting tools: `aws___call_aws` and `aws__run_script` on the AWS MCP Server, each returning a successful launch, and the built-in `request_approval`, returning approved. Selecting them in a draft run's **Run** dialog lets the workflow finish without touching AWS or waiting on a manager.
