...
| **Calling** a tool | The execution agent, mid-run | Restricted to the tools bound to a task the run currently has in progress — see [MCP gateway and proxy](./mcp-gateway-and-proxy.md) |

So a binding written here is not a hint to the model. It is the grant the [gateway](./mcp-gateway-and-proxy.md) enforces at run time, and the set the task's [certificate](./approvals.md) freezes the moment the task starts — or the moment its approval is granted, if it turns out to need one.
