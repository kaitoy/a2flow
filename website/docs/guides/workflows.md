...
**Tools that need no input approval.** Each tool's chip under **Selected Tools** carries a
small shield button beside its remove button. Click it and the values that tool is called
...
The button's icon and tint change to show the tool's current state, and hovering it names
the state and what clicking it will do.

Click it only for a tool that **reads**: a listing, a lookup, a status check. Two reasons
this exists, and both are about read-only work:
...
Leave it in its default state for anything that writes, deletes, spends, or sends. The
default is "needs approval", so a tool added and left alone keeps its input bounded. A
tool switched to "needs no approval" is labelled **Any input** on the approval the run
