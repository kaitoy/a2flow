...
## セッションのファイル {#session-files}

| 変数 | 既定値 | 役割 |
|---|---|---|
| `SESSION_FILE_MAX_BYTES` | `20971520`(20 MiB) | [ワークフローセッション](../guides/workflow-executions.md#files-in-the-session)に添付できる 1 ファイルの上限。これを超えた時点でアップロードを拒否するので、大きすぎる本文を丸ごと抱え込むことはありません |
| `SESSION_FILES_MAX_TOTAL_BYTES` | `209715200`(200 MiB) | 1 つのセッションが持てるファイルの合計サイズ。ファイルが消えるのは実行が削除されるときだけなので、長く続くセッションの上限はここで決まります |

セッションのファイルはディスクではなくデータベースに保存されます。専用のディレクトリも、レプリカ間で共有するボリュームも要らず、バックアップもデータベースのもの以外に増えません([バックアップと復元](./backup.md))。この上限を上げると、1 回の実行が占めうるデータベースの容量と、1 回のアップロードやダウンロードが使うメモリが増えます。バイト列は丸ごと読むためです。
...
## MCP のサンドボックス {#mcp-sandbox}

登録された MCP サーバーは A2Flow のコードではないので、エージェントとその秘密とは[別の場所](../architecture/mcp-proxy.md#the-sandbox)で動きます。Docker Compose の構成ではこれらは設定済みです。コンテナを自前で組む場合はここで設定します。

未設定のまま — ローカルでそのまま起動したときの状態 — だと、エージェントが自分のプロセスの中で MCP サーバーに到達します。開発では手軽ですが、誰でもサーバーを登録できる環境で望ましい状態ではありません。

| 変数 | 既定値 | 何をするか |
|---|---|---|
| `MCP_PROXY_URL` | 未設定 | サンドボックスの場所。設定することで MCP サーバーがエージェントのプロセスの外に出ます |
| `MCP_PROXY_SERVER_NAME` / `MCP_PROXY_PORT` | `mcp-proxy` / `8443` | サンドボックスに到達するための名前と、待ち受けるポート。名前は証明書の一部なので、両端で一致している必要があります |
| `MCP_PROXY_TLS_DIR` | アプリケーションの隣のディレクトリ | サンドボックスが身元を示すために必要なものを、エージェントが書き出す場所。サンドボックスは同じディレクトリを読み取り専用で参照します |
| `MCP_BACKEND_TLS_DIR` | アプリケーションの隣のディレクトリ | エージェントが**自分の**鍵を置く場所。上のディレクトリとは意図的に分けています。サンドボックスで動くものに読めてはいけないためです |
| `MCP_TRANSPORT_CERT_VALIDITY_DAYS` | `365` | それらの資材の有効期間。残り 30 日を切ると自動で再発行されるので、手を入れる必要はありません |
...
| `EMAIL_SEND_RATE_PER_SECOND` | `5.0` | リレーに渡す 1 秒あたりのメッセージ数。ワーカーは 1 通送るごとに `1 / rate` 秒待ちます。リレーの制限が厳しいときは下げてください |
