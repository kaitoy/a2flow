...
登録された MCP サーバーは A2Flow のコードではないので、エージェントとその秘密とは[別の場所](../architecture/mcp-gateway-and-proxy.md#the-sandbox)で動きます。Docker Compose の構成ではこれらは設定済みです。コンテナを自前で組む場合はここで設定します。

未設定のまま — ローカルでそのまま起動したときの状態 — だと、エージェントが自分のプロセスの中で MCP サーバーに到達します。開発では手軽ですが、誰でもサーバーを登録できる環境で望ましい状態ではありません。

| 変数 | 既定値 | 何をするか |
|---|---|---|
| `MCP_PROXY_URL` | 未設定 | プロキシの場所。設定することで MCP サーバーがエージェントのプロセスの外に出ます |
| `MCP_PROXY_SERVER_NAME` / `MCP_PROXY_PORT` | `mcp-proxy` / `8443` | プロキシに到達するための名前と、待ち受けるポート。名前は証明書の一部なので、両端で一致している必要があります |
| `MCP_PROXY_TLS_DIR` | アプリケーションの隣のディレクトリ | プロキシが身元を示すために必要なものを、エージェントが書き出す場所。プロキシは同じディレクトリを読み取り専用で参照します |
| `MCP_BACKEND_TLS_DIR` | アプリケーションの隣のディレクトリ | エージェントが**自分の**鍵を置く場所。上のディレクトリとは意図的に分けています。プロキシで動くものに読めてはいけないためです |
