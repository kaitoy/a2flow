...
## Prometheus のエンドポイント {#prometheus-endpoint}
...
## 集計サブリソース {#aggregate-sub-resources}
...
`by-approver` は承認の**宛先**で集計します。宛先はユーザーかグループのどちらかです。区別できるよう各項目は `"user"` か `"group"` の `groupKind` を持ちます。`"user"` の項目が返すのは ID だけで、`"group"` の項目は `groupLabel` にグループ名も持ちます。

失敗したタスクは**理由**を記録します。`status="failed"` と一緒に、`error_kind`(`api_error`、`timeout`、`script_error`、`invalid_input`、`permission_denied`、`rejected`、`other` のいずれか)と自由入力の `error_message` が保存されます。どちらも通常のタスクのフィールドなので、タスクの一覧をこれで絞り込めます(たとえば `?q=errorKind:eq:timeout`)。
