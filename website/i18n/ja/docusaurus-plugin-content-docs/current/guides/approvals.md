...
グループを宛先にすれば、承認が特定の 1 人の都合で止まりません。承認できるメンバーが 1 人もいないグループは、その場で拒否されます。誰も決められない依頼になってしまうからです。実行自体が持つ[アクセス制御タグ](./tags.md#execution-and-approval-tags)をすべて持っていない宛先も同様に拒否されます。その依頼を見て決裁することが永遠にできないからです。誰かが判断しても、ほかのメンバーの通知が消えることはありません。ただ、そこから操作できなくなるだけです。実際に判断した人は記録されます。グループ名だけでは「誰が承認したのか」に答えられないからです。
...
判断できるのは指名された承認者だけです。**例外はありません。宛先でない Super Admin でも判断できません**。同じ規則はタスクのステータスにも及びます。タスクのステータスを変えられるのは——手作業でも、実行エージェントに頼んでも——実行を始めた人と、**そのタスクを覆う**承認の承認者だけです。承認者が動かせるのは自分の判断が覆う範囲(承認が名指しするタスクと、その先の次の承認までのすべて)で、これが承認による実行の再開を可能にします。自分が承認を求められた範囲の外にあるタスクは進められません。そうでなければ、実行に関わる承認者の誰もが宛先の代わりを務められてしまうからです。
...
> **保証の範囲について。** MCP ゲートウェイは現在バックエンドのプロセス内で動いているので、証明書が証明できるのは、同じプロセスを共有する検証者に対する所持の事実です。すでにバックエンドを掌握した攻撃者に対する防御ではありません。得られるのは、閉じる側に倒れる単一の関門と、事後に広げられない許可と、検証できる記録です。
...
| Tags — 読み取り専用、その承認が属するワークフロー実行のタグ([ワークフロー実行と承認のタグ](./tags.md#execution-and-approval-tags)を参照) | する |
