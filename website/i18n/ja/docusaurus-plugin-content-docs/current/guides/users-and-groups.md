...
![テナントがユーザーとユーザーグループを含み、メンバーシップでユーザーがグループに属し、ユーザー自身のロールとグループのロールの両方が実効ロールになることを示すフローチャート。](./img/users-groups-relationship.svg#gh-light-mode-only)
![テナントがユーザーとユーザーグループを含み、メンバーシップでユーザーがグループに属し、ユーザー自身のロールとグループのロールの両方が実効ロールになることを示すフローチャート。](./img/users-groups-relationship-dark.svg#gh-dark-mode-only)
...
| **Enabled** | 外すと、アカウントを削除せずにサインインだけを止められます。一覧では無効なアカウントの行が淡く表示され、アバターに小さなバッジが付くので、**Enabled** 列自体はカラムピッカーで隠せます。 |
...
| **Tags** | グループを分類するラベル。ほかのタグを付けられるセクションと共有します。ここにアクセス制御タグを付けると、そのタグの付いたレコードへのアクセスを全メンバーに与えます。 |
...
**Tags** はシークレットなど、タグを付けられるほかの登録簿と共有するテナント共通の語彙です。新規作成フォームと詳細ページには **Tags** のピッカーがあり、一覧には絞り込みできる **Tags** 列があります。[タグ](./tags.md)を参照してください。グループはアクセスを与える場所でもあります。[アクセス制御タグ](./tags.md#access-control-tags)をグループに付けると、そのタグの付いたレコードがメンバーに見えるようになります。管理者は、自分がそのタグを持っていなくてもグループに付けられます。
