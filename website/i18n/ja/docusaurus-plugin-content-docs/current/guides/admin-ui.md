...
![管理 UI のナビゲーションモデルのフローチャート。ようこそページからセクションの一覧へ進み、一覧からは新規作成フォームか、名前をクリックして詳細ページを開く。詳細ページのパンくずで一覧に戻る。](./img/admin-ui-navigation.svg#gh-light-mode-only)
![管理 UI のナビゲーションモデルのフローチャート。ようこそページからセクションの一覧へ進み、一覧からは新規作成フォームか、名前をクリックして詳細ページを開く。詳細ページのパンくずで一覧に戻る。](./img/admin-ui-navigation-dark.svg#gh-dark-mode-only)
...
例外は[監査ログ](./audit-logs.md)です。こちらは読み取りも制限されているので、Admin ロールがないと直リンクを開いてもアクセス拒否の画面になります。もう 1 つの例外が[アクセス制御タグ](./tags.md#access-control-tags)の付いたレコードで、自分のグループがそのタグを持っていなければ、リンクを開いても見つからないと表示されます。
...
タスクのステータス、MCP サーバーのトランスポート、タグの色のように、決まった値しか取らない列は、アルファベット順ではなく、その値が本来読まれる順で並びます。タスクをステータスで並べ替えると `pending`、`in_progress`、`completed`、`failed`、`skipped` の順になり、これは他の画面のステータスドットが従っているライフサイクルの順と同じです。

### 列を選ぶ {#choosing-columns}
