...
ワークフロー実行は、[ワークフロー](./workflows.md)の 1 回の実行です。ワークフローの名前、説明、タスク、タグの写しを自分で持つので、元のワークフローがあとから編集されたり削除されたりしても、実行の履歴は実際に走った内容のまま残ります。

管理サイドバーの **Workflow Executions** を開くと一覧できます。ほかのセクションと違い、実行は編集するレコードではなく履歴なので、編集フォームはありません。画面はどれも、実行の内容を見せるか、実行に参加させるためのものです。**Tags** の列とフィールドが読み取り専用なのも同じ理由からです。[ワークフロー実行と承認のタグ](./tags.md#execution-and-approval-tags)を参照してください。
...
| その実行の[承認](./approvals.md)の**指名された承認者** | 同じ会話と同じ状態。承認すると、空のセッションが新しく始まるのではなく、元の実行が再開します。承認者が操作できるのは自分の承認だけです。エージェントが他のステップのために描画した入力フォームは読み取り専用になり、その下に「Waiting for the initiator to respond.」と出ます |
