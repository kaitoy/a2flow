...
このページには、その実行で行われた MCP ツール呼び出しと、[プロキシ](./workflows.md#mcp-tools-for-tasks)が下した判定が並びます。上流に届いた `allowed` と、規則に阻まれた `denied` があり、ツール名、サーバー、拒否の理由、提示された[ツール証明書](./approvals.md#human-approval)が出ます。引数はダイジェストとしてのみ表示され、生の値は保存されません。
