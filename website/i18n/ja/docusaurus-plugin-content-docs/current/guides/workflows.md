...
**入力の承認が要らないツール**。ピッカーの下の **Skip Input Approval** に、このステップに割り当てたツールがチェックボックスで並びます。チェックを入れると、そのツールを呼ぶときの値は、このステップを覆う[承認](./approvals.md)で人が同意する対象から外れます。承認そのものは変わらず必要で、ステップは承認が下りるまで待ちます。

チェックを入れてよいのは、**読むだけ**のツールです。一覧、参照、状態の確認などが該当します。理由は 2 つあり、どちらも読み取り専用の作業に固有のものです。
...
書き込む、消す、費用が発生する、送信する — こうしたツールはチェックを外したままにします。既定は未チェックなので、追加しただけのツールは入力が縛られた状態のままです。チェックしたツールは、実行時の承認で **Any input** と表示されます。どのツールが縛られていないかを、承認する人が推測せずに済むようにするためです。
