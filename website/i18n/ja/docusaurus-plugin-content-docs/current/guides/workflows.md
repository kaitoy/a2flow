...
`modified` というステータスが見えるのは **Developer**、**Reviewer**、**Super Admin** だけです。それ以外の人にはこのワークフローは `published` として見え、名前・説明・タスクテンプレートはすべて最後に公開したときのものが表示されます。未公開の編集はその人たちが見るべきものではありませんし、見えている設計こそが実際にその人の実行で動く設計です。
...
| ワークフローの詳細 | ワークフロー名をクリック | 項目の編集、**Discard changes**、**Open design session**(Developer);**Publish**、**Deactivate**(Reviewer) |
...
ワークフロー詳細ページの **Publish** が、ワークフローを実行できる状態にする操作です。これは **Reviewer** の操作です。Developer はワークフローを作って編集しますが、もう公開はしません。設計を書いた人と、それを世に出す人を分けるためです。テンプレートが 1 つ以上あり、生成が走っていないことが条件です。
...
| 編集内容が見える人 | Developer、Reviewer、Super Admin だけです |
| **Publish**(Reviewer) | 編集内容を以後の実行に反映し、他の人にも見えるようにします |
| **Discard changes**(Developer) | 編集を捨てます。テンプレートは公開済みの版から書き直され、名前は元に戻り、公開済みの版の説明が書き戻され、ワークフローは `published` に戻ります |

**Discard changes** は詳細ページのステータスバーの、Publish の隣にある取り消しアイコンです。未公開の変更がないワークフローで押しても、その旨が返るだけで何も起きません。

Developer と Reviewer のどちらのロールも持たない人には、この間ずっと公開済みの版が返ります。一覧と詳細ページの名前・説明も、**Task Templates** の画面も、さらには一覧で名前を検索したときにヒットする対象も公開済みの版です。ステータスは `published` と表示され、最後の公開より後に追加されたタスクテンプレートはそもそも存在しません。ですから作りかけの設計変更が、ワークフローを実行するだけの人を混乱させることはありませんし、その人に見えているものは常に実行される内容と一致します。

## 公開を解除する {#deactivating-a-workflow}

**Deactivate** は、ワークフローが `published` か `modified` のときに Publish の隣に現れる電源アイコンで、Publish と同じ **Reviewer** ロールが押せます。押すとワークフローは `draft` に戻ります。これで `requester` ロールの実行権限がなくなり、一度も公開していないワークフローと同じ状態になります。Developer と Super Admin はテストのために実行を続けられます。タスクテンプレート、2 つの説明フィールド、公開済みのスナップショットはそのまま残るので、もう一度公開すればすぐ元に戻ります。
...
**入力の承認が要らないツール**。**Selected Tools** に並ぶ各ツールのチップには、削除ボタンの隣に小さな盾のボタンが付いています。これを押すと、そのツールを呼ぶときの値は、このステップを覆う[承認](./approvals.md)で人が同意する対象から外れます。承認そのものは変わらず必要で、ステップは承認が下りるまで待ちます。ボタンのアイコンと色はツールの現在の状態に応じて変わり、マウスを乗せると今の状態と押した場合の動作が表示されます。

押してよいのは、**読むだけ**のツールです。一覧、参照、状態の確認などが該当します。理由は 2 つあり、どちらも読み取り専用の作業に固有のものです。

| | |
|---|---|
| 判断する材料がない | 何も変えない問い合わせの引数に、同意を求める意味がない |
| 値が事前に決まらない | 次に何をするか決めるために調べるステップは、何を尋ねるか前もって書けない |

書き込む、消す、費用が発生する、送信する — こうしたツールは既定の状態のままにします。既定は「承認が必要」なので、追加しただけのツールは入力が縛られた状態のままです。「承認が不要」に切り替えたツールは、実行時の承認で **Any input** と表示されます。どのツールが縛られていないかを、承認する人が推測せずに済むようにするためです。

**タスクのタイトル**。この 2 つの一覧はページ送りをせず全件を表示します。おかげで **Depends on** 列が相互参照として働きます。依存はタイトルで示され、そのチップにマウスを乗せると、指している行が強調されます。設計エージェントには簡潔な命令形のタイトル — 2〜4 語、50 文字 — が課されていて、チップとして読みやすく収まります。長すぎるタイトルは、詳細は説明のほうに書くよう促すメッセージとともに拒否されます。この制限がかかるのはエージェントだけです。管理フォームから編集するタイトルは 200 文字まで使えます。チップからあふれたタイトルは省略され、マウスを乗せると全体が出ます。
