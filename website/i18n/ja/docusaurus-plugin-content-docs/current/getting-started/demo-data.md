...
**AWS アカウントがない場合。** 手順 3 を飛ばし、`draft` のまま実行してください。`developer` である `demo-developer` はそれができ、ツールモックを選べる Run ダイアログが出るのはドラフト実行のときだけです。EC2 を起動するツールと、組み込みの `request_approval` を[ツールモック](../guides/tool-mocks.md)にしておけば、AWS に届くことも人の承認を待つこともなく、ワークフローが最後まで動きます。
