...
| **MCP ツール** | この手順が呼んでよい `(サーバー, ツール)` の組です。組ごとに、この手順を覆う[承認](./approvals.md)が呼び出しの値を縛るかどうかを持ちます。読むだけのツールは入力の承認が要らないものとして印を付けます。実行は自分の割り当てを編集できないので、この選択ができるのはここだけです。 |
...
| ツールの**呼び出し** | 実行中の実行エージェント | 実行がいま進行中にしているタスクに束ねられたツールだけに制限されます。[MCP ゲートウェイとプロキシ](./mcp-gateway-and-proxy.md)を参照してください |

つまり、ここで書かれたバインディングはモデルへのヒントではありません。実行時に[ゲートウェイ](./mcp-gateway-and-proxy.md)が強制する許可そのものであり、そのタスクの[証明書](./approvals.md)が、タスクが始まった瞬間に(承認が必要なタスクならその承認が下りた瞬間に)凍結する集合でもあります。
...
公開版は、ほとんどの人にとって唯一見える版でもあります。`modified` の間、未公開の名前・説明・タスクテンプレートが見えるのは **Developer** と **Super Admin** だけで、他のロールはどの画面でも公開版を読みます。ワークフロー一覧で名前を検索したり並べ替えたりするときも公開版の名前が使われ、ステータスは `published` と見えます。Developer はどちらの設計でも実行できます。公開版は本番の申請として、編集内容は[テスト実行](../guides/workflows.md#trying-the-edits-out)として実行され、後者はツールをモックでき運用メトリクスからは除外されます。
