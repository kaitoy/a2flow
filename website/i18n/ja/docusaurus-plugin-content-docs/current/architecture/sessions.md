...
- **ツール呼び出しの行** — バックエンドのツール呼び出し(`list_workflow_tasks`、`update_workflow_task` など)はすべて、スピナー(`running…`)からチェック(`done`)へ変わるコンパクトなステータス行になります。[ツールのプロキシ](./mcp-proxy.md)を通した呼び出しは、**本来の MCP ツール名**で `MCP` タグとともに出ます。`render_a2ui` と `render_approval` のクライアントツールは専用の UI を持つので、ツールの行としては出ません。
