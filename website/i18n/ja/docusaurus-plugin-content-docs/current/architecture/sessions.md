...
- **ツール呼び出しの行** — バックエンドのツール呼び出し(`list_workflow_tasks`、`update_workflow_task` など)はすべて、スピナー(`running…`)からチェック(`done`)へ変わるコンパクトなステータス行になります。[ツールのプロキシ](./mcp-proxy.md)を通した呼び出しは、**本来の MCP ツール名**で `MCP` タグとともに出ます。`render_a2ui` と `render_approval` のクライアントツール、そしてセッションのファイルを書くツールは専用の UI を持つので、ツールの行としては出ません。
...
セッションを再開したときに履歴から復元されるのは、**MCP のツール呼び出し**と**ファイルのカード**です。そのほかの A2Flow 内部のツール呼び出しと推論はその場限りです。ファイルのカードを復元するのは、ファイルがそれを作った実行より長く残り、ダウンロードのリンクが出る場所がチャットしかないからです。
