...
チェーンは最初の拒否で打ち切られ、安い判定から並びます。だからバインディングの規則で落ちる呼び出しが、署名の検証の代金を払うことはありません。最後の規則が最後にあるのには、もう 1 つ理由があります。どのタスクが呼んでいるのかを知る必要があるのはこの規則だけで、それを確定させるのが 1 つ手前の規則だからです。権限を何も持たない呼び出し元に、承認者が同意した内容を教えずに済むという効果もあります。規則を足すときは、プロキシを書き換えるのではなくチェーンにポリシーを 1 つ足します。それが「何を動かしてよいか」を 1 つの順序付きの一覧として読める状態に保ちます。
