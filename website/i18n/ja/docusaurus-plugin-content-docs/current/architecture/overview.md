...
![エージェントスキルからワークフローを生成し、ワークフローセッションの中でワークフロー実行として動かし、そのタスクが人の承認を求めるか、MCP ゲートウェイを通して MCP サーバーのツールを呼ぶことを示すフローチャート。](./img/overview-lifecycle.svg#gh-light-mode-only)
![エージェントスキルからワークフローを生成し、ワークフローセッションの中でワークフロー実行として動かし、そのタスクが人の承認を求めるか、MCP ゲートウェイを通して MCP サーバーのツールを呼ぶことを示すフローチャート。](./img/overview-lifecycle-dark.svg#gh-dark-mode-only)
...
| **実施** | ツールの呼び出しはすべて、A2Flow を出る前にポリシーチェーンで判定され、判定されたことが記録されます。 | [MCP ゲートウェイとプロキシ](./mcp-gateway-and-proxy.md) |
