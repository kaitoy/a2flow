...
ユーザーはそれぞれ**ロール**を持ち、実行できる操作がそれで決まります。ロール同士に**上下関係はありません**。例外が一つだけあります。**スーパー管理者はロールによるチェックをすべて素通り**します。そのため、ほかのどのロールとも**排他**です。あるユーザーはスーパー管理者であるか、残る 5 つのロールの組み合わせを持つかのどちらかで、両方ということはありません。この素通りには意図的な例外が 2 か所あり、いずれも所有者レベルのチェックです。どちらも後述の[セッションへのアクセスの表](./authorization.md#workflow-execution-access)に出てきます。**ロールを一つも持たない**ユーザーも有効です。自分の[アカウント](../guides/users-and-groups.md#users)(アバター)は管理できますが、それ以外は何もできません。
...
![ユーザーレコードに直接付与されたロールと、ユーザーグループから継承したロールの両方が実効ロールになり、その和集合をすべての認可チェックが使うことを示すフローチャート。](./img/authorization-effective-roles.svg#gh-light-mode-only)
![ユーザーレコードに直接付与されたロールと、ユーザーグループから継承したロールの両方が実効ロールになり、その和集合をすべての認可チェックが使うことを示すフローチャート。](./img/authorization-effective-roles-dark.svg#gh-dark-mode-only)
...
| `admin` | ユーザーの CRUD、シークレットの CRUD、ワークフロー実行の削除、そして自テナント内のすべてのワークフロー実行とそのタスク、チャット履歴、すべての承認への読み取り(後述の[ワークフロー実行とそのワークフローセッション](./authorization.md#workflow-execution-access)を参照。削除を除けば、管理者は実行のエージェントを動かしたり、タスクのステータスを変えたり、承認を決裁したりはできません) |
| `developer` | シークレットの CRUD、MCP サーバーの CRUD、[ツールモック](../guides/tool-mocks.md)の CRUD、エージェントスキルの CRUD、ワークフローの生成・編集(AI が生成した `generatedDescription` の再生成を含みます。ただしこのフィールドを直接編集できるのはスーパー管理者だけです)、タスクテンプレートの CRUD、設計セッションのチャットを動かすこと、ワークフローの実行(`POST /workflows/{id}/execute`。公開前テストのため `draft` のワークフローと、`modified` のワークフローの未公開の編集内容も実行できます)。公開と無効化は**含まれません**。それは `reviewer` の仕事です |
| `reviewer` | ワークフローの公開と無効化。`developer` がもう行わなくなった、この 2 つの操作です。ワークフローに対する `developer` と同じ読み取り範囲を共有します。`draft` のワークフロー、`modified` のワークフローの未公開の編集内容、設計セッションのチャット履歴は、レビュアーにも開発者と同じように見えます。ワークフローやそのタスクテンプレートの編集、設計セッションを動かすことはできません |
...
唯一の例外はロールではありません。[アクセス制御タグ](../guides/tags.md#access-control-tags)の付いたレコードは、自分のグループが合わせてそのタグをすべて持つユーザー、そして Admin と Super Admin(両者ともこれを素通りします)にしか見えません。それ以外の人には、レコードもその存在の手がかりも見えません。
...
| **作成フォーム**(`/<section>/new`) | アクセス拒否の画面になります。そこへ導く追加ボタンはすでに隠れているので、たどり着くのは直リンクの場合だけです |
...
- **設計セッション**はワークフローを設計するチャットです。アクセスは**ロール**で決まります。設計は開発者の仕事なので、そのテナントの開発者どうしで共有し、動かせます。レビュアーも、公開前に設計を確認するために読むことはできますが、動かすことはできません。
- **ワークフローセッション**は 1 回の実行が行われるチャットです。アクセスは**参加関係**で決まります。実行を始めた人と、その中で何かの承認を頼まれた人が、ロールに関係なく共有します。

![開発者、ワークフローの作成者、スーパー管理者が設計セッションに入れ、レビュアーはそこを読み取りのみで閲覧でき、実行の開始者、指名された承認者、スーパー管理者がワークフローセッションに入れること、そして管理者はワークフローセッションを読み取りのみできることを示すアクセスマトリクス。](./img/authorization-session-matrix.svg#gh-light-mode-only)
![開発者、ワークフローの作成者、スーパー管理者が設計セッションに入れ、レビュアーはそこを読み取りのみで閲覧でき、実行の開始者、指名された承認者、スーパー管理者がワークフローセッションに入れること、そして管理者はワークフローセッションを読み取りのみできることを示すアクセスマトリクス。](./img/authorization-session-matrix-dark.svg#gh-dark-mode-only)
...
ロールとは別に、ワークフロー実行に対する各操作には、呼び出し元がその実行の**開始者**(ワークフローを実行したユーザー)であるか、**その実行の承認のいずれかで指名された承認者**であることが必要です。指名された承認者とは、名指しされたユーザーか、承認の宛先となったグループのメンバーで `approver` を持つ人です([人による承認](../guides/approvals.md#human-approval)を参照)。承認者がチャットを共有する設計(承認者が開始者のチャットに加わる)を保ちつつ、第三者を締め出すためです。読み取りはもう少し広く、行動を伴う操作はもっと狭くなっています。
...
| タスクの**ステータス**を変える(手動でもエージェント経由でも) | `PATCH /workflow-tasks/{id}` | ✅ | 自分宛の承認が覆うタスクのみ | ❌ | どの承認も覆わないタスクのみ |
...
ある実行の**タスク一覧は、ワークフローを実行した時点で固定されます**。タスクはワークフローの公開済みテンプレートから複製され、その後は `status`(と失敗時の原因)だけが変わります。実行のタスクを API から追加・削除したり、タイトルや説明、依存関係を書き換えたりする人はいません。ステップを変えたいときは[タスクテンプレート](../guides/workflows.md#adjusting-the-task-templates)を編集して、もう一度実行します。

**最後の 3 行は二度読む価値があります。**

ステータスの行と決裁の行が、このページの冒頭で予告した**所有者レベルの例外、つまりスーパー管理者の素通りが効かない箇所**です。承認を決裁できるのは指名された承認者だけで、タスクのステータスを動かせるのは実行の開始者か、**そのタスクを覆う**承認の宛先である承認者だけです。その実行の承認者なら誰でもよいわけではなく、承認がそのタスクを覆っている場合はスーパー管理者や管理者も通りません。承認者が動かせるのは、自分が承認を求められた範囲(承認が名指しするタスクと、その先の次の承認までのステップ)だけです。これが承認によって実行を再開させつつ、承認者が無関係なステップを進めることを防ぎます。そうしないと、タスクをいきなり `completed` にすることで誰でも宛先の代わりを務められてしまいます([人による承認](../guides/approvals.md#human-approval)を参照)。手動で変えても、ワークフローセッションのチャットで実行エージェントに頼んでも、ルールは同じです。エージェントは、依頼者が直接は行えないステータス変更を拒否します。

実行の**削除**は逆の形をしています。所有者のチェックが一切ない、素のロールゲートです。そのため管理者はテナント内のどの実行でも削除できる一方、管理者ロールを持たない開始者は自分の実行すら削除できません。削除すると、その実行のタスクとワークフローセッションも一緒に消えます。
