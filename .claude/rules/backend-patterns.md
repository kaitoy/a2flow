...
`group_ids_for(caller, caller_roles=None)` takes the caller's effective roles as an *optional* fast path: call sites that already hold `EffectiveRolesDep` pass them and pay nothing, and the rest let the resolver look them up. That is what kept this change from having to thread a new dependency through five routers and three services for a check only non-participants ever reach.
