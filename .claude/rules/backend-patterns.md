...
- `dependencies/auth.py::get_access_tag_ids` → `AccessTagIdsDep` resolves the caller's set once per request (`SqlEffectiveRoleRepository.group_tag_ids_for_user`), or `None` for a `super_admin`, who is platform-scoped and can never be a group member. `None` means "unrestricted", exactly as `tenant_id=None` means "all tenants".
