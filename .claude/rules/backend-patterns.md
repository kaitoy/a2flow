...
| `SecretResolutionError` | 502 | `SECRET_RESOLUTION_FAILED` | A `${secret:NAME/KEY}` reference or skill `repo_auth_secret` could not be resolved (unknown secret, missing entry key, or a reference that omits the key); `details` carries `secret` only — the raw failure reason is logged server-side, never returned to the client |
